...
export function Logo({ className = "", showText = true }: LogoProps) {
  return (
    <div className={`flex items-center space-x-3 ${className}`}>
      <div className="relative w-12 h-12 rounded-2xl flex items-center justify-center glow-effect overflow-hidden">
        {/* Gradient background */}
      ...
      {showText && (
        <span className="text-2xl font-bold text-white">Quickbils</span>
      )}
    </div>
  );
}
